...
## Network Analysis Karate Club Example

```{r network-analysis-karate-example}
#create a table that extracts the information needed from the dataset."Turn your data into a list of connections: for each individual within each time window, find all sites where it was recorded; find all unique combinations of sites (if undirected, AB = BA); make a table so that each pair of sites is weighted based on how many times that combination was recorded"

library(zoo)
zoo::rollapply
#https://stackoverflow.com/questions/72779067/r-mutate-new-column-that-tells-me-if-any-row-in-the-next-x-number-of-row-is-be
...
sals %>% 
  ...
  arrange(date) %>% 
  arrange(pit) %>% 
  mutate(pool_from = first(pool)) %>% 
  mutate(pool_to = rollapply( #How do I extract the next pool in chronological sequence where the individual was encountered within that season?
    pool,
    width = 1,
    FUN = function
  ))

# Code below from: https://bookdown.org/jdholster1/idsr/network-analysis.html#
...
pool_net <- make_empty_graph(n = length(nodes), directed = F) %>%
  add_edges(edges) 

pool_net <- pool_net %>%
  igraph::set.vertex.attribute('group', index = V(G), value = groups)

plot(pool_net, vertex.color="green",  # Changes node color
...
How can I make a model that looks at site fidelity? First I would need to modify the data some more, creating a categorical variable for individuals that have returned to the same pool across years "return" or have not returned to the same pool across years "not return". Then I could try to predict the response of the return categorical variable based on predictors of salinity? (high/low category or just numerical)...

```{r model-fidelity}
sals %>% 
  group_by(pit) %>% 
  group_by(year) %>% 
  mutate(fidelity =  case_when(
                       ~ "yes",
                       ~ "no"
  )   #How do I create this variable? If a pool shows up more than once within a pit across seasons ~ yes, if a pool shows up once within a pit across seasons ~ no
  
```
```{r lm-returns-by-salinity}
fidelity_main_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(fidelity ~ salinity, data = sals)

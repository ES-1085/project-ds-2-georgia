...
sal_temp_change_rec <- recipe(fidelity ~ salinity_change + temp_change, 
                       data = fidel_train, family = "binomial")

sal_temp_change_wflow <- workflow() %>%
  add_model(fidelity_mod) %>%
  add_recipe(sal_temp_change_rec)

sal_temp_change_fit <- sal_temp_change_wflow %>%
  fit(data = fidel_train)
tidy(sal_temp_change_fit)
```

```{r salinity_change*temp_change model}
sal_temp_int_rec <- recipe(fidelity ~ salinity_change * temp_change, 
                       data = fidel_train, family = "binomial")

sal_temp_int_wflow <- workflow() %>%
  add_model(fidelity_mod) %>%
  add_recipe(sal_temp_int_rec)

sal_temp_int_fit <- sal_temp_int_wflow %>%
  fit(data = fidel_train)
tidy(sal_temp_int_fit)
```

...
##Create direction variables for both salinity & temp change where, if the value was negative, direction = decrease and if the value was positive, direction = increase
  ...
  mutate(sal_change_direction = if_else(salinity_change ~)) %>% 
  mutate(temp_change_direction = if_else(salinity_change)

write_csv(fidelity, "/Users/georgialattig/salamanders by the sea/data/tidy_data/fidelity.csv")
```

My data consist of 96 individual salamanders that were observed across two years. "Site fidelity" for the sake of this analysis is determined by whether the individual was observed within the same pool on the first night it was observed across 2 seasons. With these data, I will model fidelity in response to predictors of change in salinity and change in temperature. 
...
3. Does the site fidelity of individual salamanders vary with the salinity/temperature of the breeding pool they experienced during the previous season?
4. Does the site fidelity of individual salamanders vary with the breeding pool they were found in first the previous season?

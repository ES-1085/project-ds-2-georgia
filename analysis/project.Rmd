...
```{r fidelity-density-plot}
fidelity %>% 
  ggplot(aes(salinity_change, fidelity, fill = fidelity, color = fidelity)) + 
...
```{r scatterplot}
ggplot(data = fidelity,
       mapping = aes(x = salinity_change,
                     y = temp_change,
                     colour = fidelity)) +
  geom_point() +
  labs(title = "Salinity Change and Temperature Change of Breeding Pool",
       subtitle = "Across Two Years",
       x = "Salinity Change", y = "Temp Change",
       colour = "Fidelity") +
...
fidelity_rec <- recipe(factor(fidelity) ~ salinity_change + temp_change, data = fidel_train, family = "binomial")
